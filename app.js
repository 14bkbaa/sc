  ...
  function initMap() {
    ...
    if (window.GTFS && typeof window.GTFS.init === 'function') {
      window.GTFS.init().then((ok) => {
        if (ok) {
          // Ha a BKV mód már be van kapcsolva (pl. mentett állapotból), innentől
          // a gyalog szakaszoknál automatikusan használni fogjuk, ha gyorsabb.
          if (state.bkvOn) {
            showStatus('BKV mód', 'GTFS háló betöltve – tervezésnél használom, ha gyorsabb.', 2500);
          }
        } else {
          // graph.json hiányzik / nem olvasható: kapcsoljuk ki, hogy egyértelmű legyen.
          if (state.bkvOn) setBkvOn(false);
...
function clampNumber(v, a, b) { return Math.max(a, Math.min(b, v)); }
  ...
  async function loadLimoZones() {
    ...
    let data;
    try { data = evalJsLiteral(lit); } catch (e) { return; }
  ...
  function setBkvOn(on, silent=false) {
    ...
    // Ha GTFS még nem kész (betöltés alatt), ne kapcsoljuk vissza automatikusan:
    // ilyenkor a tervezés OSRM gyaloglásra esik vissza, és amint betölt, használni fogja.
    if (!silent && state.bkvOn && !(window.GTFS && window.GTFS.isReady && window.GTFS.isReady())) {
      showStatus('BKV mód', 'GTFS háló betöltése… (gtfs/graph.json)', 2500);
    }
  }
  ...
  function carIcon(isSelected=false) {
    // Smaller + more filigran icon
    const ring = isSelected ? `box-shadow:0 0 0 2px rgba(255,176,0,.28);` : '';
    const border = isSelected ? 'border:1.5px solid rgba(255,176,0,.80);' : 'border:1px solid rgba(255,255,255,.14);';
    return L.divIcon({
      className: '',
      html: `<div style="
      ...
      ">
        ...
        ">
          <svg viewBox="0 0 24 24" width="12" height="12" fill="white" opacity=".85" aria-hidden="true">
  ...
  function renderCarsOnMap() {
    // Only show cars that are explicitly checked (or have hot zones).
    // Default: ZERO cars on the map.
    for (const [id, m] of state.carsSelectedMarkers.entries()) {
      state.carsSelectedLayer.removeLayer(m);
      if (m._labelMarker) state.labelLayer.removeLayer(m._labelMarker);
    }
    state.carsSelectedMarkers.clear();

    // Legacy layers (kept for backward-compat)
    try { state.carsCluster && state.carsCluster.clearLayers(); } catch (e) {}
    try { state.carsAllMarkers && state.carsAllMarkers.clear && state.carsAllMarkers.clear(); } catch (e) {}

    for (const car of state.cars) {
      const hasZones = (state.zonesByCar[car.id] && state.zonesByCar[car.id].length);
      const shouldShow = state.selectedCars.has(car.id) || hasZones;
      if (!shouldShow) continue;

      // Accent only the "active" car in hotzone mode
      const isActive = (state.activeCarId === car.id);
      const marker = L.marker([car.lat, car.lng], { icon: carIcon(isActive), keyboard: false });
      ...
      state.carsSelectedMarkers.set(car.id, marker);
      state.carsSelectedLayer.addLayer(marker);
  ...
  function applyVisibilityRules() {
    // Plan view handles its own filtering.
    if (state.planActive) return;

    // With "selection-only" mode: ensure selected layer + labels are present.
  ...
  function onCarClick(carId) {
    ...
    try { state.carsCluster.unspiderfy && state.carsCluster.unspiderfy(); } catch (e) {}
  ...
  function renderCarList() {
    ...
    $('drawerSub').textContent = `${cars.length} autó • kijelölve: ${state.selectedCars.size}`;
    ...
    for (const c of cars) {
      ...
      main.innerHTML = `
        ...
        <div class="carMeta">${escapeHtml(c.model || '—')}</div>
  ...
  async function routeLeg(kind, from, to) {
    ...
    if (kind === 'walk' && state.bkvOn && window.GTFS && window.GTFS.isReady && window.GTFS.isReady()) {
      try {
        ...
        const req = (m >= 1200) ? 0.98 : 0.90; // hosszabb távnál engedékenyebb
        if (alt && alt.totalDuration && alt.totalDuration < approxWalk * req) {
          return {
            mode: 'bkv',
  ...
  function showPlanView(plan, opts = {}) {
    ...
    for (const step of plan.steps) {
      // walking
      const w = step.walk;
      const wColor = 'rgba(239,68,68,.95)';
      ...
      // driving
      const d = step.drive;
      const dLine = L.polyline(d.geometry.map(p => [p.lat, p.lng]), { color: 'rgba(59,130,246,.95)', weight: 7, opacity: 0.9 });
    ...
    plan.steps.forEach((s, i) => {
      ...
      el.className = 'step';
      const isBkv = (s.walk.mode === 'bkv');

      const asc = (!isBkv && s.walk.ascent != null && s.walk.descent != null)

        ? ` • ${s.walk.ascent} m ↗ ${s.walk.descent} m ↘`
      ...
      if (isBkv && s.walk.meta) {

        const lines = Array.isArray(s.walk.meta.lines) ? s.walk.meta.lines :

                      (typeof s.walk.meta.line === 'string' ? [s.walk.meta.line] :

                      (typeof s.walk.meta.route === 'string' ? [s.walk.meta.route] : []));

        const uniq = Array.from(new Set((lines || []).filter(Boolean))).sort();

        const lineTxt = uniq.length ? ` • járat: ${escapeHtml(uniq.join(', '))}` : '';

        const stopTxt = (s.walk.meta.startStop && s.walk.meta.endStop)

          ? ` • ${escapeHtml(s.walk.meta.startStop)} → ${escapeHtml(s.walk.meta.endStop)}`

          : '';

        const cntTxt = (typeof s.walk.meta.stops === 'number') ? ` • ${s.walk.meta.stops} megálló` : '';

        bkvExtra = `${lineTxt}${stopTxt}${cntTxt}`;

      }


      const walkMode = isBkv ? '🚇/🚌' : '🚶';

      const walkMeta = `${walkMode} ${fmtMin(s.walk.duration)} • ${s.walk.distance != null ? fmtKm(s.walk.distance) : '—'}${asc}${bkvExtra}`;
